...
const readThemeStructure = function readThemeFiles(themePath, subPath, arr, deps = {}) {
    const fsImpl = deps.fs || fs;
    const osImpl = deps.os || os;
    const pathImpl = deps.path || path;
    const normalizePathFn = deps.normalizePath || normalizePath;

    themePath = pathImpl.join(themePath, '.');
    subPath = subPath || '';

    var tmpPath = osImpl.tmpdir(),
    ...
    var makeResult = function makeResult(result, subFilePath, ext, symlink) {
        result.push({
            file: subFilePath,
            normalizedFile: normalizePathFn(subFilePath),
            ext,
            symlink
        });
        return result;
    ...
    return fsImpl.readdir(themePath, {withFileTypes: true}).then(function (files) {
        let result = arr;
        ...
        return files.reduce(function (promise, dirent) {
            return promise.then(function () {
                ...
                const subFilePath = pathImpl.join(subPath, file);
                const newPath = pathImpl.join(themePath, file);
                ...
                if (dirent.isDirectory()) {
                    return readThemeStructure(newPath, subFilePath, result, deps)
                        .then((updatedResult) => {
 ...
 */
const readFiles = function readFiles(theme, deps = {}) {
    const fsImpl = deps.fs || fs;
    const pathImpl = deps.path || path;
    const astLinter = deps.ASTLinter || ASTLinter;
    const processHelpersFn = deps.processHelpers || processHelpers;
    ...
    return Promise.all(themeFilesContent.map((themeFile) => {
        return fsImpl.readFile(pathImpl.join(theme.path, themeFile.file), 'utf8').then(function (content) {
            ...
            if (handlebarsMatch) {
                ...
                processHelpersFn(theme, themeFile, deps);
            }
        });
    })).then(() => theme);
};

const processHelpers = function (theme, themeFile, deps = {}) {
    const helperLinter = deps.linter || linter;
    ...
    for (const helper of helperLinter.helpers) {
