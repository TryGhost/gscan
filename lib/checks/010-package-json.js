...
const symmetricDifference = (arrayA, arrayB) => {
    ...
    return [
        ...(arrayA || []).filter(value => !setB.has(value)),
        ...(arrayB || []).filter(value => !setA.has(value))
    ];
};
...
_private.validatePackageJSONFields = function validatePackageJSONFields(packageJSON, theme, packageJSONValidationRules) {
    ...
    theme.results.pass = theme.results.pass.concat(symmetricDifference(failed, Object.values(objectWithout(packageJSONValidationRules, passedRulesToOmit))));
