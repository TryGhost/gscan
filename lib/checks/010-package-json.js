...
const objectWithout = (object, keysToOmit) => {
...
const v1PackageJSONValidationRules = Object.assign({
...
const v4PackageJSONValidationRules = Object.assign({},
...
const v5PackageJSONValidationRules = Object.assign({},
...
const v6PackageJSONValidationRules = Object.assign({},
...
_private.validatePackageJSONFields = function validatePackageJSONFields(packageJSON, theme, packageJSONValidationRules) {
    ...
    if (!packageJSON.config || packageJSON.config.posts_per_page === null || typeof packageJSON.config.posts_per_page === 'undefined') {
        markFailed('configPPPIsRequired');
        passedRulesToOmit.push('configPPIsInteger');
    } else if (typeof packageJSON.config.posts_per_page !== 'number' || packageJSON.config.posts_per_page < 1) {
        failed = failed.filter(rule => rule !== packageJSONValidationRules.configPPPIsRequired);
    ...
    if (!packageJSON.keywords || !Array.isArray(packageJSON.keywords) || !packageJSON.keywords.includes('ghost-theme')) {
    ...
    if (!packageJSON.config || (isEmptyValue(packageJSON.config.card_assets) && typeof packageJSON.config.card_assets !== 'boolean')) {
    ...
    Object.entries(failedRules).forEach(([key]) => {
    ...
    const activeRules = Object.values(objectWithout(packageJSONValidationRules, passedRulesToOmit));
    const failedSet = new Set(failed);
    const passedRules = activeRules.filter(rule => !failedSet.has(rule));

    // Use set difference (activeRules - failed) instead of symmetricDifference.
    theme.results.pass = theme.results.pass.concat(passedRules);
...
module.exports = async function checkPackageJSON(theme, options, themePath) { // eslint-disable-line no-unused-vars
    ...
    if (checkVersion === 'v1') {
        ...
        packageJSONValidationRules = mergeObjects(
            v1PackageJSONValidationRules,
            v2PackageJSONValidationRules
        );
        packageJSONConditionalRules = mergeObjects(
            v1PackageJSONConditionalRules,
            v2PackageJSONConditionalRules
        );
    ...
    let [packageJSON] = theme.files.filter(file => file.file === packageJSONFileName);
    ...
    if (packageJSON && packageJSON.content) {
        try {
            ...
            Object.assign(theme.results.fail, _private.getFailedRules(Object.values(objectWithout(packageJSONValidationRules, ['isPresent'].concat(Object.keys(packageJSONConditionalRules))))));
        ...
        const rules = _private.getFailedRules(Object.values(objectWithout(packageJSONValidationRules, Object.keys(packageJSONConditionalRules))));
