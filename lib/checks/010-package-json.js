...
/**
 *  detectPackageJSONerrors:
 *    If uploading/activating a theme on a Ghost production blog, we don't want to detect package.json errors,
 *    because this can be very annoying for a theme customer. For gscan.ghost.org or Ghost development we have
 *    to show the errors - these errors are displayed for the theme developer!
 */
module.exports = function checkPackageJSON(theme, themePath, options) {
    options = _.extend({
        detectPackageJSONErrors: true
    }, options || {});

    var packageJSONPath = path.join(theme.path, packageJSONFile);

    // CASE: detecting package.json errors can be disabled (see reasons in lib/index.js)
    if (!options.detectPackageJSONErrors) {
        return theme;
    }
