...
const checkTemplatesCompile = function checkTemplatesCompile(theme, options) {
    ...
    const rulesToCheck = Object.fromEntries(Object.entries(ruleSet.rules).filter(([ruleCode]) => ruleCode.match(ruleRegex)));

    const processFile = processFileFunction(theme.files, failures, theme, partialsFound);

    Object.entries(rulesToCheck).forEach(([ruleCode]) => {
