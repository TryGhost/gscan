...
function processFileFunction(files, failures, rules, partialVerificationCache, verifyInvocationCount) {
    ...
    return function processFile(linter, themeFile) {
        ...
        // Count the invocation of linter.verify
        verifyInvocationCount.count = verifyInvocationCount.count + 1;
...
const checkTemplateSyntax = function checkTemplateSyntax(theme, options) {
    ...
    // Initialize the cache and count within this function scope
    const verifyInvocationCount = {count: 0};
    ...
    console.log(`Total time for 090-template-syntax check: ${ed - st}ms`);  //eslint-disable-line
    console.log(`Total linter.verify invocations: ${verifyInvocationCount.count}`); //eslint-disable-line
    verifyInvocationCount.count = 0;
