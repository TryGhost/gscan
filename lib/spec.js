...
rules = {
    ...
    "GS001-DEPR-FAV": {
        "level": "error",
        "rule": "The usage of <code>{{asset 'favicon'}}</code> in HTML <code>head</code> is no longer required",
        "details": "The usage of <code>{{asset 'favicon'}}</code> in the HTML <code>head</code> tag is no longer required because Ghost outputs this for you automatically in <code>{{ghost_head}}</code>.<br>" +
                   "To see, what else is rendered with the <code>{{ghost_head}}</code> helper, look <a href=\"https://themes.ghost.org/docs/ghost_head\" target=_blank>here</a>."
    },
    'GS001-DEPR-PURL': {
        level: 'error',
