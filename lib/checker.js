...
const check = async function checkAll(themePath, options = {}) {
    ...
    const originalHelpers = spec.knownHelpers;
    const extraHelpers = Object.entries(labsEnabledHelpers)
        .filter(([, flag]) => options.labs && options.labs[flag])
        .map(([helper]) => helper)
        .filter(helper => !originalHelpers.includes(helper));

    if (extraHelpers.length > 0) {
        spec.knownHelpers = [...originalHelpers, ...extraHelpers];
    }
    ...
    try {
        ...
        if (extraHelpers.length > 0) {
            spec.knownHelpers = originalHelpers;
        }
    }
};
