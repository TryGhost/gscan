...
let rules = {
    ...
    'GS090-NO-TIER-PRICE-AS-OBJECT': {
        level: 'error',
        fatal: true,
        rule: 'Replace <code>{{monthly_price.*}}</code> with <code>{{price}}</code> and <code>{{@member.subscriptions}}</code>',
        details: oneLineTrim`The <code>{{monthly_price}}</code> helper was removed in favor of <code>{{price}}</code> and <code>{{@member.subscriptions}}</code><br>
        ${tierDesc}<br>
        Find more information about the <code>{{price}}</code> helper <a href="${docsBaseUrl}members/#the-price-helper" target=_blank>here</a>.`,
        helper: '{{monthly_price}}'
    },
