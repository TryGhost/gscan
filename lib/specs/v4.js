...
const previousRules = structuredClone(previousSpec.rules);
...
knownHelpers = Array.from(new Set([...previousKnownHelpers, ...knownHelpers]));
templates = Array.from(new Set([...previousTemplates, ...templates]));
...
const mergedRules = {...previousRules};
Object.entries(rules).forEach(([ruleCode, ruleConfig]) => {
    mergedRules[ruleCode] = {...(mergedRules[ruleCode] || {}), ...ruleConfig};
});
rules = mergedRules;
Object.values(rules).forEach(function replaceDocsUrl(value) {
