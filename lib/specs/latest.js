...
rules = {
    ...
    'GS001-DEPR-BLG': {
        level: 'error',
        rule: 'The <code>{{@blog}}</code> block helper should be replaced with <code>{{@site}}</code>',
        details: `With the introduction of Content API <code>{{@blog}}</code> became deprecated and <code>{{@site}}</code> is now preferred.<br>
        <code>{{@blog}}</code> helper will be removed in next version of Ghost and should not be used.
    ...
    'GS001-DEPR-LNG': {
        level: 'error',
        rule: 'The <code>{{lang}}</code> block helper should be replaced with <code>{{@site.lang}}</code>',
        details: `The <code>{{lang}}</code> helper is duplicate to {{@site.lang}} and the latter is preferred method to access <br>
        <code>{{lang}}</code> helper will be removed in next version of Ghost and should not be used.
        Find more information about the <code>@site.lang</code> property <a href="${docsBaseUrl}blog" target=_blank>here</a>.`,
        regex: /{{\s*?lang\s*?}}/g,
        helper: '{{lang}}'
    },
    'GS001-DEPR-CSS-KGMD': {
