...
let previousKnownHelpers = previousSpec.knownHelpers;
let previousTemplates = previousSpec.templates;
let previousRules = previousSpec.rules;

// Register any helpers, templates, or rules that should not be valid anymore
let knownHelpersToRemove = []; // e. g. ['author', 'pagination']
let templatesToRemove = []; // e. g. ['Private template', ...]
let rulesToRemove = ['GS002-DISQUS-ID']; // e. g. ['GS001-DEPR-MD', ...]

// Reduce our previous spec by the rules we want to delete
_.each(knownHelpersToRemove, function (helper) {
    _.pull(previousKnownHelpers, helper);
});

_.each(templatesToRemove, function (template) {
    _.remove(previousTemplates, function (props) {
        return props.name === template;
    });
});
console.log('​previousRules before', Object.keys(previousRules).length);

previousRules = _.omit(previousRules, rulesToRemove);
console.log('​previousRules after', Object.keys(previousRules).length);
...
rules = {
    ...
    'GS001-DEPR-BPL': {
        level: 'error',
        rule: 'The usage of <code>{{@blog.permalinks}}</code> is deprecated',
