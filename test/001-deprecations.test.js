    ...
    describe('latest version:', function () {
        it('[failure] theme is completely invalid (v1 and latest)', function (done) {
            utils.testCheck(thisCheck, '001-deprecations/latest/invalid_all').then(function (output) {
                ...
                // {{@blog}}
                output.results.fail['GS001-DEPR-BLG'].should.be.a.ValidFailObject();
                output.results.fail['GS001-DEPR-BLG'].failures.length.should.eql(2);
        ...
        it('[failure] theme is invalid for latest version', function (done) {
            utils.testCheck(thisCheck, '001-deprecations/latest/invalid').then(function (output) {
                ...
                // {{@blog.*}}
                output.results.fail['GS001-DEPR-BLG'].should.be.a.ValidFailObject();
                output.results.fail['GS001-DEPR-BLG'].failures.length.should.eql(1);
                ...
                // {{lang}}
                output.results.fail['GS001-DEPR-LNG'].should.be.a.ValidFailObject();
                output.results.fail['GS001-DEPR-LNG'].failures.length.should.eql(1);
